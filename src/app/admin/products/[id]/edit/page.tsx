...
export default function EditProduct({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      try {
        ...
        setPriceSource(data.price_source || "manual");
        ...
        setTags(fetchedTags.filter((t: string) => t !== 'Best Selling').join(', '));
        setIsBestSelling(fetchedTags.includes('Best Selling'));
  ...
  const handleSave = async (overrideStatus?: string) => {
    ...
    try {
      ...
      // Fetch previous product data before saving to compare price
      const { data: prevProd } = await supabase.from('products').select('current_price, currency').eq('id', id).single();
      const oldVal = prevProd && prevProd.current_price !== null ? Number(prevProd.current_price) : null;
      const newVal = currentPrice ? parseFloat(currentPrice) : null;
      ...
      if (oldVal !== newVal && newVal !== null) {
        await supabase.from('price_history').insert([{
          ...
          source: 'manual',
          note: 'Price updated in step-based editor'
        }]);
      }
