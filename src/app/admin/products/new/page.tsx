  ...
  useEffect(() => {
    async function fetchAndSeedCategories() {
      ...
      try {
        ...
        dbFetchedCats = existingCats || [];
      } catch (err) {
        console.error("Failed to fetch categories from database:", err);
      }

      // If we couldn't fetch anything from DB, fall back to core static categories with mock UUIDs so the dropdown is NEVER empty or stuck loading!
      if (dbFetchedCats.length === 0) {
        console.warn("Falling back to static core categories list...");
        dbFetchedCats = coreCategories.map((cc, i) => ({
          id: `static-fallback-id-${i}`,
          ...cc
        }));
      }

      let finalCats = [...dbFetchedCats];

      // Try to seed missing categories and clean up legacy ones in background.
      // If client-side writes are blocked by RLS policies, it will fail silently here but NOT break the fetched list!
      try {
        // Only seed/migrate if we actually connected to database successfully (i.e. did not use static-fallback ids)
        const isDbConnected = dbFetchedCats.length > 0 && !dbFetchedCats[0].id.startsWith('static-fallback');
        
        if (isDbConnected) {
          const missingCats = coreCategories.filter(cc => !dbFetchedCats.some(ec => ec.slug === cc.slug));
          if (missingCats.length > 0) {
            ...
            if (!insertErr && insertedCats) {
              ...
              console.warn("Seeding missing categories blocked or failed:", insertErr?.message);
            }
          }
        ...
        console.error("Seeding/migration in background encountered write restriction:", err);
      }
    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-200/80 dark:border-slate-800">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white">Add New Product</h1>
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Sleek step-based affiliate product builder</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {saveSuccess && (
            <div className="px-4 py-2.5 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900/40 text-green-600 dark:text-green-400 text-sm rounded-xl flex items-center gap-2 font-medium">
              <CheckCircle2 className="w-4 h-4 shrink-0" /> Product added successfully!
            </div>
          )}
        </div>
      </div>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col justify-between">
                      ...
                      <input type="file" ref={fileInputRef} onChange={handleFileUpload} modifier-upload-hidden="true" multiple accept="image/*" className="hidden" />
                      <div 
                        ...
                        {['Students', 'Creators', 'Gamers', 'Working Professionals', 'Setup Lovers', 'Everyday Buyers'].map(aud => (
                          <label key={aud} className={cn(
                            ...
                            audience.includes(aud)
                              ...
                              : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-850 hover:border-slate-300 text-slate-650 dark:text-slate-350"
                          )}>
          ...
          <div className="sticky top-8 space-y-6">
            ...
            <div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200/85 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xl shadow-slate-250/20 dark:shadow-none flex flex-col min-h-[440px]">
                {/* Image panel */}
                <div className="aspect-[4/3] bg-slate-50 dark:bg-slate-850 relative w-full flex items-center justify-center p-6 border-b border-slate-100 dark:border-slate-800">
